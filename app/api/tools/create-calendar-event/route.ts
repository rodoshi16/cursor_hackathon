...
export async function POST(req: Request) {
    ...
    return safeJsonResponse(
      {
        ok: false,
        error:
          "This interview does not have a clear date and time. It cannot be added to the calendar.",
      },
      { status: 400 }
    );
  }
  if (interview.confidence < 0.6) {
    return safeJsonResponse(
      {
        ok: false,
        error: "Confidence too low to safely add this to the calendar.",
      },
      { status: 400 }
    );
  }
  if (hasCalendarEventForEmail(interview.provider, interview.emailId)) {
    return safeJsonResponse({ ok: true, duplicate: true, interview });
  }
  ...
  try {
    if (isDemoMode()) {
      ...
      return safeJsonResponse({ ok: true, interview: updated });
    }
    ...
    const eventRes = await client.createCalendarEvent(account, {
      ...
      timezone: interview.timezone || "America/Toronto",
